...
gssh = 1.65
    ...
    Nanj = Formulas.Nanj(M, N)       '// Наибольшее растягивающее усилие в одном ряду анкеров  (формула (Б.2) [1])
    Qanj = Formulas.Qanj(Q, M, N)      '// Сдвигающее усилие, приходящееся на один ряд анкеров (формула (Б.3) [1])
...
Dim minL As Double
    ...
    Nanj = Formulas.Nanj(M, N)       '// Наибольшее растягивающее усилие в одном ряду анкеров  (формула (Б.2) [1])
    Qanj = Formulas.Qanj(Q, M, N)      '// Сдвигающее усилие, приходящееся на один ряд анкеров (формула (Б.3) [1])
    Nan1 = Nanj / EMDD.n_Anchor '// Растягивающие и сдвигающие усилия в одном анкерном стержне
    Qan1 = Qanj / EMDD.n_Anchor '// Растягивающие и сдвигающие усилия в одном анкерном стержне
    w = 0.7
    Dl = 11
    If Nan1 <= 0 Then
        jc = 0.7
    ...
    lan = Formulas.lan(w, Dl, jc, Q, M, N) '//Требуемая длина анкеровки lan арматурного стержня периодического профиля без усиления (п.5.7 [2]):
    ...
    If (EMDD.La_Anchor < 15 * EMDD.dAnchor) Then '// Локальная сила на усиление анкера Nloc
        Nloc = Nan1 + Qan1 * (15 * EMDD.dAnchor - EMDD.La_Anchor) / lan '// Для La (длина анкера) < 15d (Диаметр анкера (мм))
    Else
